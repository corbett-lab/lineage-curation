// lineageEditing.js — client-side port of the Linolium backend lineage-editing
// endpoints (server.js: /merge-lineage, /edit-lineage-root, /undo-edit,
// /undo-preview, /edit-history, /lineages). Every operation is an in-memory
// transform on the Taxonium node array (processedData.nodes), byte-for-byte the
// same logic the Express backend ran. No network, no protobuf round-trip.
//
// A single module-level editHistory + originalLineages snapshot mirrors the
// backend's process-global state. bindEditing(processedData, field) captures the
// baseline lineage assignment (for the export "modified" column) once at load.

let editHistory = [];
let originalLineages = null; // Map node_id -> original lineage, captured at first bind
...
// ---- shared helpers (ported verbatim from server.js) --------------------
...
function rebuildCladeLabels(processedData, field) {
  ...
  for (const [lineage, tips] of Object.entries(lineageTips)) {
    ...
    // the tip itself, so it is dropped on rebuild — this matches the reference
    // backend, which also sheds single-tip lineages whenever it recomputes
    // clade labels (golden: 263 initial → 257 after merging XFG.5 = 263 − 4
    // merged − 2 single-tip lineages dropped on rebuild).
...
export function undoPreview(targetId) {
  return { targetId, wouldUndo: computeConflictingEdits(targetId) };
}

export function undoEdit(processedData, id) {
  ...
  for (let i = toUndo.length - 1; i >= 0; i--) {
    ...
    for (const node of processedData.nodes) {
      if (node.is_tip && entry.snapshot.hasOwnProperty(node.node_id)) node[entry.field] = entry.snapshot[node.node_id];
    }
  }
